...
func BuildReport(cfg *config.Config, cfgPath string, dataDir string, healthResult *health.HealthResult, coreState string, now time.Time) Report {
	...
	if cfg.Apps.Mode == "all" {
		appendFinding(
			...
			"Весь обычный трафик отправляется через RKNnoVPN. Это лучший режим для сетей с белыми списками; чувствительные встроенные исключения всё равно обходят перехват на уровне UID.",
			"LOW",
			"ROUTING",
			"Если нужна максимальная изоляция отдельных приложений, переключитесь на белый список и выберите приложения вручную.",
			"apps.mode",
		)
	}
