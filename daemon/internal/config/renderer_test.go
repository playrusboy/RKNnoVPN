...
func TestRuntimeProfileNodeDiagnosticsReportsSkippedNodes(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Node.Address = ""
	cfg.Node.UUID = ""
	cfg.Profile.ActiveNodeID = "good-vless"
	...
	report := RuntimeProfileNodeDiagnostics(cfg)
	if report.Total != 3 || report.Usable != 2 || report.Renderable != 1 {
		t.Fatalf("unexpected runtime node report: %#v", report)
	}
	if len(report.Skipped) != 2 {
		t.Fatalf("expected stale and invalid nodes to be reported, got %#v", report.Skipped)
	}
	if report.Skipped[0].ID != "stale-node" || !strings.Contains(report.Skipped[0].Reason, "stale") {
		t.Fatalf("stale node skip not reported clearly: %#v", report.Skipped[0])
	}
	if report.Skipped[1].ID != "bad-ss" || !strings.Contains(report.Skipped[1].Reason, "shadowsocks method") {
		t.Fatalf("invalid outbound skip not reported clearly: %#v", report.Skipped[1])
	}
}
